...
if(isset($_POST['submit']))
{
	...
	{
		$sql ="INSERT INTO prescription(treatment_records_id,doctorid,patientid,prescriptiondate,status,appointmentid) values('$_POST[treatmentid]','$_POST[select2]','$_POST[patientid]','$_POST[date]','Active','$_POST[appointmentid]')";
		if($qsql = mysqli_query($conn,$sql))
		{
			...
			echo "<script>window.location='prescriptionrecord.php?prescriptionid=" . $insid . "&patientid=$_GET[patientid]&appid=$_GET[appointmentid]';</script>";
		}
		else
		{
			echo mysqli_error($conn);
